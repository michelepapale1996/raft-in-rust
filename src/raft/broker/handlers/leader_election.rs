...
impl LeaderElectionHandler {
    ...
    async fn trigger_leader_election(&self, raft_state: &mut RaftState) {
        ...
        let request_vote_responses_by_hosts = self
            .request_executor
            .perform_vote_request(&request_vote_request)
            .await;
        let request_vote_responses = request_vote_responses_by_hosts
            .values()
            .collect::<Vec<&RequestVoteResponse>>();

        tracing::info!(
            "Received {} responses out of {} nodes during the election process",
            request_vote_responses.len(),
            cluster_hosts.len()
        );
    ...
    fn gained_quorum(
        cluster_size: usize,
        request_vote_responses: Vec<&RequestVoteResponse>,
    ) -> bool {
        let number_of_approvals = request_vote_responses
            .iter()
    ...
    fn build_request_vote_requests(
        raft_state: &mut RaftState,
    ) -> HashMap<String, RequestVoteRequest> {
