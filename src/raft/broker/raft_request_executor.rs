...
impl RaftRequestExecutor {
    ...
    pub async fn perform_append_entries_requests(
        &self,
        append_entries_requests_by_host: &HashMap<String, AppendEntriesRequest>,
    ) -> HashMap<String, AppendEntriesResponse> {
        self.perform_requests("append_entries", append_entries_requests_by_host)
            .await
    }

    pub async fn perform_vote_request(
        &self,
        request_vote_request: &HashMap<String, RequestVoteRequest>,
    ) -> HashMap<String, RequestVoteResponse> {
        self.perform_requests("request_vote", request_vote_request)
            .await
    }

    async fn perform_requests<T, U>(
        &self,
        endpoint_suffix: &str,
        request_bodies_by_host: &HashMap<String, T>,
    ) -> HashMap<String, U>
    where
        T: serde::Serialize,
        U: serde::de::DeserializeOwned + std::fmt::Debug,
    {
        ...
        for (cluster_host, request_body) in request_bodies_by_host {
            ...
            let res = self.client.post(&endpoint).json(&request_body).send().await;
