...
use crate::raft::model::state::{RaftNodeConfig, RaftState};
...
pub async fn start(node_config: RaftNodeConfig) {
    ...
    leader_election_timeout_emitter
        .start(election_timeout_seconds)
        .await;
    ...
    append_entries_timeout_emitter
        .start(heartbeat_timeout_seconds)
        .await;

    tracing::info!("Starting raft request acceptor...");
    let raft_request_acceptor = RaftRequestAcceptor::new(bus_tx.clone());
    raft_request_acceptor
        .start_accepting_requests(&node_config)
        .await;
